/**
 * Author: Kyle
 * GameOverScreen component - displays final results and leaderboard with avg time + correct count.
 */
    ...
    >
      ...
      >
        ...
        <p className="mt-6 text-center text-xl text-white/80">
          Your Score:{" "}
          <span className="font-heading text-5xl text-smart-green">{score}</span>
        </p>
          ...
          <table className="w-full border-collapse text-left text-sm sm:text-base">
            <thead className="bg-black/20 uppercase tracking-[0.25em] text-white">
              <tr>
                ...
                <th className="px-4 py-3 text-right">Avg Time (s)</th>
              </tr>
            </thead>
            <tbody>
              {finalLeaderboard.map((row, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right text-white/90 hidden sm:table-cell">
                    {row.correct ?? "-"}
                  </td>
                  <td className="px-4 py-3 text-right text-white">{row.correct ?? "-"}</td>
                  <td className="px-4 py-3 text-right text-white">{row.avgTime ? row.avgTime.toFixed(1) : "-"}</td>
                </tr>
              ))}
            </tbody>
          </table>
