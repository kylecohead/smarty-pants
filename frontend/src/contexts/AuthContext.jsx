...
export function AuthProvider({ children }) {
  ...
  const logTokenStatus = (context = '') => {
    ...
    console.log(`Access Expires: ${accessExp ? accessExp.toLocaleString() : 'Unknown'} (${formatTimeUntilExpiration(accessExp)})`);
    console.log(`Refresh Token: ${refreshToken ? refreshToken.substring(0, 20) + '...' : 'NONE'}`);
    console.log(`Refresh Expires: ${refreshExp ? refreshExp.toLocaleString() : 'Unknown'} (${formatTimeUntilExpiration(refreshExp)})`);
  ...
  const fetchCurrentUser = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('⏳ AuthContext: Access token expired (401), attempting refresh...');
        // Try to refresh token
        const refreshSuccess = await refreshToken();
        if (refreshSuccess) {
          console.log('✅ AuthContext: Token refresh successful, retrying user fetch...');
        ...
        console.log(`❌ AuthContext: Unexpected response status ${response.status}, clearing auth state`);
        clearAuthState();
      }
    } catch (error) {
  ...
  const handleLoginSuccess = (userData) => {
    console.log('🎉 AuthContext: Login success for user:', userData.username || userData.id);
    ...
    const interval = setInterval(() => {
      ...
      if (isAuthenticated()) {
        ...
        if (timeUntilExpiry > 0 && timeUntilExpiry < 2 * 60 * 1000) {
          console.warn(`⚠️ AuthContext: Access token expires soon! (${formatTimeUntilExpiration(accessExp)})`);
        }
        
        // Check if token is already expired - try to refresh first
        if (timeUntilExpiry <= 0) {
          console.error('💀 AuthContext: Access token has expired! Attempting refresh...');
          (async () => {
            try {
              const refreshSuccess = await refreshToken();
              if (refreshSuccess) {
                console.log('✅ AuthContext: Successfully refreshed expired token');
                logTokenStatus('(after periodic refresh)');
              } else {
                console.error('❌ AuthContext: Failed to refresh expired token, clearing auth state');
                clearAuthState();
              }
            } catch (error) {
        ...
        console.log('🔄 AuthContext: User was logged in but no valid token found, clearing state');
        clearAuthState();
      }
  ...
  const validateAuthStatus = () => {
    ...
    if (timeUntilExpiry <= 0) {
      console.warn('⚠️ AuthContext: Token expired during sync validation (will attempt refresh elsewhere)');
      return false;
    }

    console.log(`✅ AuthContext: Auth status valid (expires in ${formatTimeUntilExpiration(expiration)})`);
    return true;
  };

  // Function to validate and refresh authentication (async, attempts refresh if needed)
  const validateAndRefreshAuth = async () => {
    console.log('🔍 AuthContext: Validating and refreshing auth status (async)...');
    ...
    if (timeUntilExpiry <= 0) {
      console.error('💀 AuthContext: Token expired during async validation, attempting refresh...');
      try {
        const refreshSuccess = await refreshToken();
        if (refreshSuccess) {
          console.log('✅ AuthContext: Successfully refreshed expired token during validation');
          logTokenStatus('(after validation refresh)');
          return true;
        } else {
          console.error('❌ AuthContext: Failed to refresh expired token during validation');
          clearAuthState();
          return false;
        }
      } catch (error) {
        console.error('❌ AuthContext: Error during token refresh in validation:', error);
        clearAuthState();
        return false;
      }
    }
  ...
  const getValidAccessToken = async () => {
    ...
    const token = getAccessToken();
    const expiration = decodeTokenExpiration(token);
    console.log(`✅ AuthContext: Returning access token (expires: ${formatTimeUntilExpiration(expiration)})`);
